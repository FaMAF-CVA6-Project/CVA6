...
`clean_gem5_runs.py` takes `results/m5out/`, `results/run/`, `results/batch/`, the sweep result folders, `results/parity/`, and `__pycache__/`. `clean_CVA6_runs.py` takes `verif/sim/out_<date>/`, `work-ver/`, `results/batch/`, `results/sweep_CVA6Flow/`, `results/run/`, `results/verif/`, and `__pycache__/`. Extra folders can be named on the command line, for a run made with a custom `--gem5-out-dir` or `--out-dir`.
