...
| [`viewers/MinorFlow/`](https://github.com/FaMAF-CVA6-Project/MinorFlow#readme)       | The MinorFlow viewer, as a submodule.                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| [`viewers/CVA6Flow/`](https://github.com/FaMAF-CVA6-Project/CVA6Flow#readme)         | The CVA6Flow viewer, as a submodule.                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `viewers/FlowCompare.html`                                                           | The side-by-side comparison viewer, a tracked file of this repository rather than a submodule.                                                                                                                                                                                                                                                                                                                                                                                           |
| [`viewers/pairs/`](viewers/pairs/pairs.js)                                           | The program pairs FlowCompare's Load sample pair offers, gem5 against the RTL, written by `scripts/make_flowcompare_pairs.py`.                                                                                                                                                                                                                                                                                                                                                           |
| [`.github/workflows/flowcompare-pages.yml`](.github/workflows/flowcompare-pages.yml) | Publishes FlowCompare and its pairs as this repository's GitHub Pages site. The other workflows are upstream's, and disabled in this fork.                                                                                                                                                                                                                                                                                                                                               |
...
| `scripts/make_flowcompare_pairs.py`                                                  | Writes `viewers/pairs/`, the gem5 and RTL pairs FlowCompare offers, from two folders of tracer JSONs, each side gzipped inside a script.                                                                                                                                                                                                                                                                                                                                                 |
...
`viewers/FlowCompare.html` puts the two side by side and measures the drift between them. It takes one JSON of each kind, dropped onto the page together or chosen slot by slot, and `metadata.tool` decides which slot each belongs in.

**Load sample pair** offers the programs in `viewers/pairs/`: each one on the matched gem5 configuration beside the same program on the RTL, 13 of the calibration set, the four whose JSONs pass 50 MiB left out. Each side is the tracer JSON gzipped inside a script, 19.5 MiB for all 13 against 200 MiB of JSON, which the page unpacks with the browser's own gzip support, served or opened from disk. [`scripts/make_flowcompare_pairs.py`](scripts/make_flowcompare_pairs.py) writes them from two folders of tracer JSONs, by default `container_results/review/patch/` and `container_results/review/rtl/`, and a second run replaces them. Without `viewers/pairs/`, the button falls back to every program both viewers have a sample of in their `tests/` folders.

```bash
python3 scripts/make_flowcompare_pairs.py                               # rebuild viewers/pairs/
python3 viewers/MinorFlow/scripts/serve_MinorFlow.py --root viewers    # then open /FlowCompare.html
```

FlowCompare is also this repository's GitHub Pages site. [`.github/workflows/flowcompare-pages.yml`](.github/workflows/flowcompare-pages.yml) publishes `viewers/FlowCompare.html` as the front page with `viewers/pairs/` beside it, whenever either changes on `master`, and nothing else from the repository, since upstream's tree holds symlinks that point nowhere, which Pages refuses. Its Pages source is set to GitHub Actions.
