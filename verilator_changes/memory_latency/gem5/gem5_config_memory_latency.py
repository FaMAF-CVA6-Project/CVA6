...
# Memory latency sweep harness for the CVA6 gem5 configuration. The core is
# identical to the production configuration in every respect. The only field
# that moves is the main-memory latency.
#
# RTL pairing. Each entry names the Verilator build it must be compared
# against. That build carries axi_lat_delayer_intf in ariane_testharness.sv at
# the stated MemLatencyCycles, which injects that many cycles on the DRAM read
# address channel, or 20 ns per cycle at the 50 MHz harness clock. 
# The stock PULP axi_delayer cannot produce these operating points: its
# stream_delay elements are per-handshake stalls, so they charge the delay on
# every burst beat and serialise the channel they delay, and the vendored copy
# silently truncates any delay above 15 because its counter is 4 bits wide.
#
#   MEM  gem5 latency  RTL point                 role
#     1  60ns          MemLatencyCycles = 0      calibrated, baseline
#     2  140ns         MemLatencyCycles = 5      calibrated
#     3  260ns         MemLatencyCycles = 10     calibrated
#     4  460ns         MemLatencyCycles = 20     calibrated
#     5  160ns         MemLatencyCycles = 5      bracket
#     6  220ns         MemLatencyCycles = 5      bracket
#     7  360ns         MemLatencyCycles = 10     bracket
#     8  660ns         MemLatencyCycles = 20     bracket

MEM = 1

# (name, gem5 memory latency, MemLatencyCycles of the paired Verilator build)
MEM_POINTS = {
    1: ("D0 calibrated",  "60ns",  0),
    2: ("D5 calibrated",  "140ns", 5),
    3: ("D10 calibrated", "260ns", 10),
    4: ("D20 calibrated", "460ns", 20),
    5: ("D5 bracket lo",  "160ns", 5),
    6: ("D5 bracket hi",  "220ns", 5),
    7: ("D10 bracket hi", "360ns", 10),
    8: ("D20 bracket hi", "660ns", 20),
}
...
parser = argparse.ArgumentParser(description="CVA6 memory latency sweep on gem5")
parser.add_argument("binary", type=str,
                    help="Path to the compiled RISC-V ELF binary")
args = parser.parse_args()

if MEM not in MEM_POINTS:
    raise ValueError(
        f"MEM={MEM} is not a sweep point. Valid IDs: {sorted(MEM_POINTS.keys())}")
point_name, mem_latency, rtl_cycles = MEM_POINTS[MEM]

print("=" * 70)
print(f"   CVA6 MEMORY LATENCY SWEEP  -  MEM {MEM}: {point_name}")
print(f"   gem5 memory latency : {mem_latency}")
print(f"   compare against RTL : MemLatencyCycles = {rtl_cycles} "
      f"({rtl_cycles * 20} ns injected)")
...
# Bandwidth is set high so the penalty stays latency-dominated, and
# latency_var is zero so the sweep varies one quantity only.
