...
def print_table(results, clean_file=None):
    # Buffer temporal para almacenar el texto de salida
    output_buffer = []
    ...
    output_buffer.append("="*70 + "\n")
    output_buffer.append(f"Clean result (OFICIAL):  {clean_array_official}")
    ...
    # Adjuntar la tabla y listas al final del archivo específico del programa
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Ejecutar simulacion gem5 en RISC-V para programas C y consolidar reportes.")
    parser.add_argument("config_file", help="Ruta al archivo de configuracion de gem5 (.py)")
    parser.add_argument("c_file", help="Ruta al archivo de codigo C (.c)")
    ...
    # Obtener el nombre base del programa (ej: "mi_programa" desde "tests/mi_programa.c")
    program_name = os.path.splitext(os.path.basename(c_file))[0]
