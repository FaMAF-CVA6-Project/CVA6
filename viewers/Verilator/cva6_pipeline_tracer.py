...
def tag_branch_bubbles(records):
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            delta = nxt.if1_lo - causer.if1_lo
            if delta > 1:
                bubble_size = delta - 1
...
class PipelineTracker:
    ...
    def attribute_dc_events_to_records(self):
        ...
        for ev in evlog:
            ...
            n_miss_total += 1
            sid = ev.get("sid")
            if sid == LOAD_UNIT_SID:    n_miss_loads_g += 1
            elif sid == STORE_ADAPTER_SID: n_miss_stores += 1
            else:                        n_miss_other  += 1
...
def stream_and_extract(f, matches, args, n_wb_ports, n_commit_ports):
    ...
    for n in range(NR_SB):
        ...
        bp_cf_vid = single_id.get(f"issue_stage_i.i_scoreboard.mem_q[{n}].sbe.bp.cf")
        MEMQ_FU[n] = f_vid
        MEMQ_RS1[n] = r1_vid
        MEMQ_RS2[n] = r2_vid
        MEMQ_RD[n] = rd_vid
                    ...
                    if ptr_id is not None:
                        ...
                        if tid is not None:
                            ...
                            if MEMQ_BP_AVAILABLE and 0 <= tid < NR_SB:
                                mq_bp_cf = binary_to_int(state.get(MEMQ_BP_CF[tid]))
        ...
        if DV and DA and state.get(DV) == "1" and state.get(DA) == "1":
            ...
            if not flush_unissued:
                ...
                if tid is not None:
                    ...
                    # forwarding pattern to the new issuer (the
                    # compress.json "addi t4 forwarding to addi t5"
                    # bug). idx_hzd_rsX has the same timing.
                    if FWD_AVAILABLE:
                        ...
                        ihz_rs1_v = binary_to_int(state.get(IHZ_RS1)) if IHZ_RS1 else None
                        ihz_rs2_v = binary_to_int(state.get(IHZ_RS2)) if IHZ_RS2 else None
                        ihz_rs3_v = binary_to_int(state.get(IHZ_RS3)) if IHZ_RS3 else None
                    else:
...
def write_output_json(output_path, args, stats, tracker):
    ...
    with output_path.open("w") as f:
        ...
        for i, ev in enumerate(allocs):
            ...
            row = {"cycle": ev["cycle"], "sid": ev.get("sid"), "pf": ev.get("pf", 0)}
...
def main():
    ...
    elapsed = time.time() - start
    # Surface the derived clock period so the user can sanity-check
    # the time-base conversion. CVA6 testharness sims commonly run
    # at 10 ns / 100 MHz, which shows as 10000 ps here.
    ...
    if cp_ts is not None:
        ...
        unit_to_ps = {"1fs":1e-3, "1ps":1, "1ns":1e3, "1us":1e6, "1ms":1e9, "1s":1e12}
        ps = cp_ts * unit_to_ps.get(ts_unit.strip(), 1)
        if ps >= 1e6:    period_disp = f"{ps/1e6:.3f} us"
        elif ps >= 1e3:  period_disp = f"{ps/1e3:.3f} ns"
        else:            period_disp = f"{ps:.0f} ps"
